...
//  HeartBreakSkate
//
//  Created by Shanice Brown on 4/16/19.
...
// which object types colliede or have contact with each other
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //An array that holds all the current sidewall bricks
    ...
    // Timestamp of the last update method call
    ...
    override func didMove(to view: SKView) {
        ...
        //Set up the skater and add him to the scene
        skater.setupPhysicsBody()
        addChild(skater)
        
        // Add tap gester recognizer to know when the user tapped the screen
        let tapMethod = #selector(GameScene.handleTap(tapGesture:))
        let tapGesture = UITapGestureRecognizer(target: self, action: tapMethod)
        view.addGestureRecognizer(tapGesture)
        
    // Add a menu overlay with "Touch to Start" text
        ...
        menuLayer.display(message: "Touch to Start", score: nil)
        addChild(menuLayer)
    }
    ...
    func setupLabels() {
        // Label that says "score" in the upper left corner
        ...
        scoreTextLabel.fontName = "Menlo-Bold"
        ...
        highScoreTextLabel.fontName = "Menlo-Bold"
    ...
    func removeGem(_gem: SKSpriteNode) {
        
        _gem.removeFromParent()
        
        if let gemIndex = gems.index(of: _gem) {
    ...
    func updateBricks(withScrollAmount currentScrollAmount: CGFloat){
        ...
        while farthestRightBrickX < frame.width {
            ...
            if randomNumber < 2 && score > 10 {
                ...
                // There is a 2 percent chance that we will
                // leave a gap between bricks after the player has reached a score of 10
    ...
    func updateScore(withCurrentTime currentTime: TimeInterval) {
        ...
        // The player's score increases the longer they stay on the board
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        // Determine the elasped time since the last update call
        var elaspedTime: TimeInterval = 0.0
        if let lastTimeStamp = lastUpdateTime {
            elaspedTime = currentTime - lastTimeStamp
        }
        lastUpdateTime = currentTime
        
        let expectedElaspedTime: TimeInterval = 1.0 / 60.0
        
        //  We calculate how far everything should move in this update
        let scrollAdjustment = CGFloat(elaspedTime / expectedElaspedTime)
    ...
    @objc func handleTap(tapGesture: UITapGestureRecognizer) {
        ...
        // Make the skater jump if player taps while skater is on the ground
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        // Check if the contact is between the skater and the brick
        if contact.bodyA.categoryBitMask == PhysicsCategory.skater && contact.bodyB.categoryBitMask == PhysicsCategory.brick {
            ...
            // Skater touched a gem, remove it
            if let gem = contact.bodyB.node as? SKSpriteNode {
                removeGem(_gem: gem)
                
                // Give player 100 points for getting a gem
                score += 100
                updateScoreLabelText()
